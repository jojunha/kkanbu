...
{
  kkanbu_msgs::SensorPointArray spa;
  spa.obs_info.clear();

  for (auto c : lidar_info.circles) {
    int tres; 
    tres = c.radius + car_radius + 2;
    ...
    if (tres >= c.c_distance)
      {
        if(c.center.x < 0)
        {
          if((c.angle > -0.53) &&(c.angle<0.53))
          {
          
            sp.x=c.center.x;
            sp.y=c.center.y;
            sp.r=c.radius;
            spa.obs_info.push_back(sp);
          
          }
        }
      }
  }
  final_pub.publish(spa);
}
